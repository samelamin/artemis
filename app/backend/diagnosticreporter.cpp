...
quint32 crc32Update(quint32 crc, const char *data, qsizetype len)
{
    static const quint32 kTable[256] = {
    ...
    for (qsizetype i = 0; i < len; ++i) {
        crc = (crc >> 8) ^ kTable[(crc ^ static_cast<quint8>(data[i])) & 0xffu];
    }
    return ~crc;
}
...
{
    // qCompress returns: 4-byte BE original size, 2-byte framing header,
    // raw deflate stream, 4-byte BE adler32. We strip the framing and emit
    // a real gzip stream with a deterministic (MTIME=0) header and a proper
    // CRC32 + ISIZE trailer.
    //
    // Empty input is a contract bug rather than a production P0 because
    // every report we send is non-empty. Handle it deterministically so
    // downstream tooling, including the system gzip -d decoder and the
    // tests, gets a real 20-byte gzip stream instead of an empty one.
    if (input.isEmpty()) {
        QByteArray empty;
        empty.reserve(20);
        empty.append(static_cast<char>(0x1f));
        empty.append(static_cast<char>(0x8b));
        empty.append(static_cast<char>(0x08)); // CM = deflate
        empty.append(static_cast<char>(0x00)); // FLG: no extra fields
        empty.append(static_cast<char>(0x00)); // MTIME[0]
        empty.append(static_cast<char>(0x00)); // MTIME[1]
        empty.append(static_cast<char>(0x00)); // MTIME[2]
        empty.append(static_cast<char>(0x00)); // MTIME[3]
        empty.append(static_cast<char>(0x00)); // XFL
        empty.append(static_cast<char>(0xff)); // OS = unknown
        // Deflate payload for empty input: a single final fixed-Huffman
        // block containing only the end-of-block symbol. Two bytes:
        // 0x03 = BFINAL=1, BTYPE=01, then 5 reserved zero bits; 0x00 =
        // the remaining 2 bits of the end-of-block code plus 6 zero
        // padding bits. CRC32 of empty input is 0; ISIZE is 0.
        empty.append(static_cast<char>(0x03));
        empty.append(static_cast<char>(0x00));
        empty.append(static_cast<char>(0x00)); // CRC32 byte 0
        empty.append(static_cast<char>(0x00)); // CRC32 byte 1
        empty.append(static_cast<char>(0x00)); // CRC32 byte 2
        empty.append(static_cast<char>(0x00)); // CRC32 byte 3
        empty.append(static_cast<char>(0x00)); // ISIZE byte 0
        empty.append(static_cast<char>(0x00)); // ISIZE byte 1
        empty.append(static_cast<char>(0x00)); // ISIZE byte 2
        empty.append(static_cast<char>(0x00)); // ISIZE byte 3
        return empty;
    }
    const QByteArray zlibFrame = qCompress(input, 9);
    if (zlibFrame.size() < 10) {
        // 4-byte size prefix + 2-byte header + 4-byte adler = minimum 10 bytes.
        return QByteArray();
    }
    const int deflateStart = 6;
    const int deflateEnd = zlibFrame.size() - 4;
    if (deflateEnd <= deflateStart) {
        return QByteArray();
    }

    const int deflateLen = deflateEnd - deflateStart;
    const quint32 crc = crc32Update(0u, input.constData(), input.size());
    const quint32 inputSizeMod = static_cast<quint32>(
        static_cast<quint64>(input.size()) & 0xffffffffu);

    QByteArray output;
    output.reserve(10 + deflateLen + 8);
    output.append(static_cast<char>(0x1f));
    output.append(static_cast<char>(0x8b));
    output.append(static_cast<char>(0x08)); // CM = deflate
    output.append(static_cast<char>(0x00)); // FLG: no extra fields
    output.append(static_cast<char>(0x00)); // MTIME[0]
    output.append(static_cast<char>(0x00)); // MTIME[1]
    output.append(static_cast<char>(0x00)); // MTIME[2]
    output.append(static_cast<char>(0x00)); // MTIME[3]
    output.append(static_cast<char>(0x00)); // XFL
    output.append(static_cast<char>(0xff)); // OS = unknown
    output.append(zlibFrame.constData() + deflateStart, deflateLen);
    output.append(static_cast<char>(crc & 0xffu));
    output.append(static_cast<char>((crc >> 8) & 0xffu));
    output.append(static_cast<char>((crc >> 16) & 0xffu));
    output.append(static_cast<char>((crc >> 24) & 0xffu));
    output.append(static_cast<char>(inputSizeMod & 0xffu));
    output.append(static_cast<char>((inputSizeMod >> 8) & 0xffu));
    output.append(static_cast<char>((inputSizeMod >> 16) & 0xffu));
    output.append(static_cast<char>((inputSizeMod >> 24) & 0xffu));
    return output;
}
